...
def run_showmap(pdf_input, output):
    ...
    #pre_split="cat %s | cut -d ':' -f1 > %s" % (tmp_output, output)
    #os.system(pre_split)
...
def trim_pdf(seed):
    ...
    logging.info('Summary of file %s: remove %d objects' % (base_name_seed, succeed_cnt))
    logging.info('Summary of file %s: pdf size(original): %d bytes' % (base_name_seed, orig_pdf_size))
    logging.info('Summary of file %s: pdf size(after trimming): %d bytes' % (base_name_seed, trimed_pdf_size))
    logging.info('Summary of file %s: optimized rate: %f' % (base_name_seed, optimized_rate))
