...
def recursive_remove(dom_parser, soup):
    """
    resursivly remove elements from sub-tree to tree
    ...
    for (idx, child) in enumerate(children):
        ...
        # only when all children nodes are removed safely,
        # we can remove current node
        parse &= recursive_remove(cur_dom_parent, soup)
    ...
    for idx in removed_idx:
        ...
        if not test_valid_trim(soup.prettify()):
            ...
            if not test_valid_trim(soup.prettify()):
                ...
                logging.info('[Trim valid Str Replacing]: replace %s with %s'\
                        % (repr(replaced_str), shorter_str))
        else:
            global SUCCEED_CNT_STR
            SUCCEED_CNT_STR += 1
            logging.info('[Trim valid Str]: removed %s'\
                    % (repr(removed_str)))
            succeed_cnt += 1
