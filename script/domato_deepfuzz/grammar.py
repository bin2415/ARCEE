...
class Grammar(object):
    ...
    def _expand_rule(self, symbol, rule, context,
        ...
        for part in rule['parts']:
            ...
            if part['type'] == 'text':
                ...
                if expanded.strip() in DEEPFUZZ_TAGS and random.random() < DEEPFUZZ_RATE:
                    logging.debug('Use deepfuzz for tag %s' % expanded)
                    use_deepfuzz = True
        ...
        if use_deepfuzz:
            ...
            if len_prev_content >= 150:
                prev_content_150 = context['content'][-150:]
            else:
                prev_content_150 = 'a' * (150 - len_prev_content)
                prev_content_150 += context['content']
            ret_v = deepfuzz_generate.generate(prev_content_150, deepfuzz_tag)
            context['lines'].append(ret_v)
            context['content'] += ret_v.strip()
            logging.debug('deepfuzz generate: %s' % ret_v)
            return ret_v
        ...
        if rule['type'] == 'grammar':
            context['content'] += filed_rule.strip()
            return filed_rule
        else:
