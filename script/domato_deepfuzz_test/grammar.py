...
import logging
import sys
sys.path.append('/src/sync/DeepFuzz_backup1230')
import generate_tag
#import deepfuzz_generate

logging.basicConfig(format = '%(asctime)-15s %(levelname)s:%(message)s', level = logging.INFO)

input_characters = {'进', '呵', '<', '\\', ')', 'в', 'o', '步', '频', '业', '相', '已', '2', '、', '水', '，', 'サ', '有', 'u', '本', 'こ', '入', 'い', '0', 'ケ', '页', '必', '3', 'q', '→', '定', '月', '‘', 'F', "'", 'マ', 'ィ', '–', '無', '素', 'N', '当', 'ь', '界', '名', '日', '图', '#', 'フ', '%', '!', 'が', '钟', 'ち', '复', '—', 'グ', '藏', '看', 'ü', '愛', 'e', ']', 'У', 'D', 'も', 'の', '能', 'g', '*', '主', '想', 'é', 'х', 'V', 'お', 'I', 'ц', '回', '商', '分', '出', '的', '.', '8', '"', 'C', 's', '年', 'し', 'ッ', 'R', '扱', 'ム', '画', 'ソ', '/', '合', 'L', '号', '`', 'ル', 'б', '章', '】', 'り', '>', '用', 'ё', '工', '新', '：', '（', '收', 'プ', 'く', 'に', 'A', '视', '_', '来', '技', 'a', '像', 'В', '说', '…', '备', 'ラ', '楼', 'к', '限', 'ろ', 'っ', '報', 'ч', '及', 'т', '速', '=', 'パ', 'n', 'h', 'f', 'p', '直', '(', 'x', 'な', '×', '部', 'р', '下', '›', '“', ':', 'и', ' ', '題', '材', '4', 'ク', '／', '文', '利', 'P', '更', 'm', 'M', '”', 'з', '片', '那', '首', 'す', 'B', 'н', 'е', '除', '去', 'Р', 'ф', '{', '未', 'ю', 'は', 'ш', '気', 'Y', '然', 'ェ', 'Ó', 'ま', 'G', 'k', 'с', '·', '现', '･', 'ж', 'よ', '。', '5', '6', 'ウ', 'ズ', 'H', '起', 'ト', '@', '’', 'K', 'l', '|', 'ア', '自', '大', '好', '知', 'O', 'П', '理', 'ы', '一', '組', '£', '经', '＠', 'た', 'イ', '处', 'ブ', '$', 'r', '^', '拡', '方', '™', 'b', 'さ', ';', '~', '又', '沪', 'ó', '途', '类', 'ン', 'м', '【', 'へ', '只', 'て', 'ö', '&', '語', '+', '?', ',', 'а', 'W', '版', '総', 'й', 'я', 'ら', 'レ', '简', '软', '似', 'か', 'j', 'X', 'd', 'Z', '}', '接', 'U', 'ジ', 'у', '放', 'щ', 'で', '不', '？', 'う', '1', '9', '使', 'S', 'д', 'デ', 'T', '款', '人', '番', 'w', 'И', '▼', 'だ', 'z', 'ス', '超', '解', 't', '样', '体', '約', 'ロ', '中', 'c', 'Q', 'á', 'г', '©', '✓', 'i', 'を', '在', 'E', '話', 'п', 'ノ', 'о', 'y', 'と', 'ぶ', '载', 'あ', 'v', '-', 'ん', 'J', '腹', 'ä', '7', '了', 'ー', '‣', 'る', '做', '[', '错', '术', 'л', '勲', '火', '件', '没', 'れ', 'ñ', 'ぱ', 'э'}
...
# rate that use tag generated by deepfuzz
DEEPFUZZ_RATE = 0.2
CUR_RECURSION_DEPTH_MASK = -1
...
class Grammar(object):
    ...
    def _generate_import(self, tag):
        ...
        if 'symbol' in tag:
            symbol = tag['symbol']
            return grammar.generate_symbol(symbol)[0]
        else:
    ...
    def _generate(self, symbol, context,
        ...
        if (symbol in context['variables'] and
            ...
            if (force_var_reuse or
                ...
                ret_v = variables[random.randint(0, len(variables) - 1)]
                return (ret_v, ret_v)
    ...
    def _expand_rule(self, symbol, rule, context,
        ...
        Raises:
            ...
            RecursionError: If maximum recursion level was reached.
        """
        global CUR_RECURSION_DEPTH_MASK
        global DEEPFUZZ_CONTENT
        use_deepfuzz = False
        deepfuzz_tag = ''
        variable_ids = {}
        ...
        for part in rule['parts']:
            if 'id' in part:
                if part['id'] in variable_ids:
                    ...
                    deepfuzz_parts.append(variable_ids[part['id']])
                    continue

            if part['type'] == 'text':
                expanded = part['text']
                deepfuzz_expanded = part['text']
                if expanded.strip() in DEEPFUZZ_TAGS and random.random() < DEEPFUZZ_RATE:
                    use_deepfuzz = True
                    deepfuzz_tag = expanded.strip()
                    CUR_RECURSION_DEPTH_MASK = recursion_depth
                ...
                deepfuzz_expanded = expanded
            else:
                try:
                    (expanded, deepfuzz_expanded) = self._generate(
                        part['tagname'],
                        context,
                        recursion_depth + 1,
                        force_nonrecursive
            ...
            deepfuzz_parts.append(deepfuzz_expanded)

        # this part is generated by deepfuzz
        if use_deepfuzz:
            len_prev_content = len(context['content'])
            if len_prev_content >= 30:
                prev_content_30_tmp = context['content'][-30:]
            else:
                prev_content_30_tmp = ' ' * (30 - len_prev_content)
                prev_content_30_tmp += context['content']

            prev_content_30 = ''

            for c in prev_content_30_tmp:
            ...
            idx = 0
            while idx < 5:
                ret_v = generate_tag.generate(prev_content_30, deepfuzz_tag)
                if ret_v.strip() != '':
                    break
                idx += 1
            context['content'] += ret_v.strip()
            logging.info('Use deepfuzz for tag %s, deepfuzz generate: %s, prev_content_30 is %s' %\
                    (deepfuzz_tag, ret_v, prev_content_30))
            #return ret_v
        ...
        deepfuzz_content = ret_v if use_deepfuzz else ''.join(deepfuzz_parts)
        if rule['type'] == 'grammar':
            if not use_deepfuzz:
                context['content'] += filed_rule
            return (filed_rule, deepfuzz_content)
        else:
            ...
            if symbol == 'line':
                ...
                ret_v = ret_vars[random.randint(0, len(ret_vars) - 1)]
                if not use_deepfuzz:
                    context['content'] += ret_v
                return (ret_v, deepfuzz_content)
