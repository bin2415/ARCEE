...
def generate_new_sample(template, htmlgrammar, cssgrammar, jsgrammar):
    ...
    result = template
    deepfuzz_result = template

    grammar.DEEPFUZZ_CONTENT=''
    #css = cssgrammar.generate_symbol('rules')
    (html, deepfuzz_html) = htmlgrammar.generate_symbol('bodyelements')
    ...
    deepfuzz_html = re.sub(
        r'<[a-zA-Z0-9_-]+ ',
        lambda match: add_html_ids(match, htmlctx2),
        deepfuzz_html
    )
    generate_html_elements(htmlctx, _N_ADDITIONAL_HTMLVARS)

    generate_html_elements(htmlctx2, _N_ADDITIONAL_HTMLVARS)

    #result = result.replace('<cssfuzzer>', css)
    deepfuzz_result = deepfuzz_result.replace('<htmlfuzzer>', deepfuzz_html)
...
def generate_samples(grammar_dir, outfiles):
    ...
    for outfile in outfiles:
        (result, deepfuzz_result) = generate_new_sample(template, htmlgrammar, cssgrammar,
                                     jsgrammar)
        
        deepfuzz_out = outfile.replace('.html', '_deepfuzz.html')
        ...
        if deepfuzz_result is not None:
            print('Writing a sample to ' + deepfuzz_out)
            try:
                f = open(deepfuzz_out, 'wb')
                f.write(deepfuzz_result.encode())
                f.close()
            except IOError:
                print('Error writing to output')
